...
module internal FableClientEmitter =
    ...
    let emit (iface: RpcInterfaceInfo) (types: SerdeTypeInfo list) : string =
        ...
        // F# disallows multi-file additions to a top-level module. If the user's
        // [<RpcApi>] interface is declared inside `module Foo.Bar`, we cannot emit
        // a sibling file under the same `module Foo.Bar` path. Surface a clear,
        // actionable error rather than the cryptic FS0247 the user would otherwise see.
        if not iface.IsParentNamespace then
            let out = StringBuilder()
            let oappend (s: string) = out.Append(s).Append('\n') |> ignore
            oappend "// <auto-generated />"
            oappend (sprintf "namespace SerdeGenerated.Fable.%s" iface.ShortName)
            oappend ""
            oappend (sprintf "#error \"Serde.FS [<GenerateFableClient>] requires '%s' to be declared inside a 'namespace', not a top-level 'module'. Change 'module %s' to 'namespace %s' (and wrap any top-level let-bindings in a sub-module).\"" iface.FullName nsPart nsPart)
            oappend ""
            out.ToString()
        else
